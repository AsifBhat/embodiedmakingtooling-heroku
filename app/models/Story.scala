...
object Story {
  var allStories = List(
    new Story("S0001", "We share 10 meeting rooms, usually named after people to identify them, between 300 people, and the meeting rooms need to be kept locked."),
  ...
  def all(): List[Story] = allStories
  
  def getElementById(id: String): Option[Story] = all().find(_.id == id)
}
